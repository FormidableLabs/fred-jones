...
// All subsequent files required by node with the extensions .es6, .es, .jsx and .js will be
...
function renderReport(outputDir, options) {
  var template = _.template(fs.readFileSync(__dirname + '/client/index.html').toString());
  var data = {
    report : getReport(outputDir),
  ...
  fs.writeFileSync(path.join(outputDir, 'report.svg'), reactTemplate(data), 'utf8');

  var renderedTemplate = template(data);
  fs.writeFileSync(path.join(outputDir, 'index.html'), renderedTemplate, 'utf8');
}
...
exports.inspect = function(files, outputDir, options, done) {
  ...
  files = _.flatten(_.map(files, function(file) { return glob.sync(file); }));
  ...
  // Tone down logging if in quiet mode
  ...
  if (!fileOutputDir) {
    runReports(files, flags, options)
      .then(done);
  } else {
    runReports(files, flags, options)
      .then(function(reports) {
        fs.copy(assets, path.join(outputDir, 'assets'), function() {
          ...
          writeReport(outputDir, 'report', overviewReport);

          renderReport(outputDir, {
            title : options.title,
            flags : flags
          });
